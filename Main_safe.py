...
if USE_STREAM:
    ...
    _csv_size_mb = Path(BASE_PATH, LFP_FILENAME).stat().st_size / 1e6 if Path(BASE_PATH, LFP_FILENAME).exists() else 0
    print(f"[INFO] Lade CSV ({_csv_size_mb:.0f} MB) – bitte warten ...", flush=True)
...
if not FROM_STREAM:
    ...
    try:
        ...
        print(f"[MUA][WARN] {_e_mua}")
else:
    NUM_CHANNELS = LFP_array.shape[0]
    mua_rate_hz = np.nan
    mua_spike_times = np.array([], dtype=float)
    print("[MUA] Streaming-Modus: MUA nicht verfügbar (kein Rohdaten-Zugriff)")
...
# ===== Probe Channel Map =====
# CHANNEL_MAP=5,3,1,0,2,4,...  → Datenkanal-Indizes in physischer Reihenfolge (0 = Spitze/tiefste Elektrode)
# Damit wird LFP_array so umgeordnet dass Zeile 0 = tiefste physische Elektrode.
_chan_map_spec = os.environ.get("CHANNEL_MAP", "").strip()
if _chan_map_spec:
    try:
        _chan_map = [int(x.strip()) for x in _chan_map_spec.split(",")]
        _n = LFP_array.shape[0]
        if len(_chan_map) == _n and sorted(_chan_map) == list(range(_n)):
            LFP_array = LFP_array[_chan_map, :]
            print(f"[CHANNEL_MAP] angewendet: {len(_chan_map)} Kanäle umgeordnet")
        elif len(_chan_map) > _n or max(_chan_map) >= _n:
            print(f"[CHANNEL_MAP][WARN] Map-Einträge außerhalb 0-{_n-1} — ignoriert")
        else:
            print(f"[CHANNEL_MAP][WARN] Map ist keine vollständige Permutation von 0-{_n-1} — ignoriert")
    except Exception as _e:
        print(f"[CHANNEL_MAP][WARN] konnte CHANNEL_MAP='{_chan_map_spec}' nicht parsen: {_e}")
...
cfg = {
    "balanced": {
        ...
        "artifact_z": 7.0,
        "artifact_frac": 0.015,
        "line_ratio_max": 0.15,   # 50Hz: >15% Power → schlecht
        "hf_ratio_max": 0.40,     # HF-Rauschen: >40% → schlecht
        "std_rel_min": 0.15,
        "std_rel_max": 5.0,
        "corr_min": 0.05,         # nur grob positive Korrelation nötig
        "jump_ratio_max": 1.00,
    },
    "strict": {
...
_chan_metrics = {}
for i in candidate_idx:
    x = np.asarray(LFP_array[i], float)
    finite_frac = float(np.isfinite(x).mean())
    std = float(np.nanstd(x))
    valid = np.isfinite(std) and std > 0
    rel_std = float(std / std_med) if (valid and np.isfinite(std_med) and std_med > 0) else _nan
    art_frac = float(np.mean(np.abs((x - np.nanmedian(x)) / std) > cfg["artifact_z"])) if valid else _nan
    line_ratio = _line_noise_ratio(x, fs) if valid else _nan
    hf_ratio = _hf_noise_ratio(x, fs) if valid else _nan
    jump_ratio = _jump_ratio(x) if valid else _nan
    quasi_bin = _is_quasi_binary_trace(x) if valid else False
    _chan_metrics[i] = dict(
        finite_frac=finite_frac, std=std, rel_std=rel_std,
        art_frac=art_frac, line_ratio=line_ratio, hf_ratio=hf_ratio,
...
for i in candidate_idx:
    m = _chan_metrics[i]
    if m["finite_frac"] < cfg["min_finite_frac"]:
        _pass1_bad.add(i); continue
    if not np.isfinite(m["std"]) or m["std"] == 0:
        _pass1_bad.add(i); continue
    if np.isfinite(m["rel_std"]):
        if m["rel_std"] < cfg["std_rel_min"] or m["rel_std"] > cfg["std_rel_max"]:
            _pass1_bad.add(i); continue
    if m["quasi_bin"]:
        _pass1_bad.add(i); continue
    if np.isfinite(m["art_frac"]) and m["art_frac"] > cfg["artifact_frac"]:
        _pass1_bad.add(i); continue
    if np.isfinite(m["line_ratio"]) and m["line_ratio"] > cfg["line_ratio_max"]:
        _pass1_bad.add(i); continue
    if np.isfinite(m["hf_ratio"]) and m["hf_ratio"] > cfg["hf_ratio_max"]:
        _pass1_bad.add(i); continue
    if np.isfinite(m["jump_ratio"]) and m["jump_ratio"] > cfg["jump_ratio_max"]:
        _pass1_bad.add(i); continue

# Pass 2: Template aus Pass-1-Überlebenden → Korrelationscheck aussagekräftig
_pass1_good = [j for j in candidate_idx if j not in _pass1_bad]
if len(_pass1_good) >= 2:
    template = np.nanmedian(np.asarray(LFP_array[_pass1_good], float), axis=0)
    for i in _pass1_good:
        _chan_metrics[i]["corr"] = _corr_to_template(np.asarray(LFP_array[i], float), template)
else:
    template = np.nanmedian(np.asarray(LFP_array[candidate_idx], float), axis=0)

# Endgültiger Filter: Pass-1-Ergebnisse übernehmen + Korrelationscheck
for i in candidate_idx:
    m = _chan_metrics[i]
    if i in _pass1_bad:
        if m["finite_frac"] < cfg["min_finite_frac"]:
        ...
        if np.isfinite(m["rel_std"]):
            if m["rel_std"] < cfg["std_rel_min"]:
                bad_idx.add(i); reasons.append((i, f"sehr niedriges Sigma(rel={m['rel_std']:.2f})")); continue
            if m["rel_std"] > cfg["std_rel_max"]:
                bad_idx.add(i); reasons.append((i, f"sehr hohes Sigma(rel={m['rel_std']:.2f})")); continue
        if m["quasi_bin"]:
            bad_idx.add(i); reasons.append((i, "quasi-binär")); continue
        if np.isfinite(m["art_frac"]) and m["art_frac"] > cfg["artifact_frac"]:
            bad_idx.add(i); reasons.append((i, f"Artefakte ({m['art_frac']*100:.1f}% |z|>{cfg['artifact_z']:.1f})")); continue
        if np.isfinite(m["line_ratio"]) and m["line_ratio"] > cfg["line_ratio_max"]:
            bad_idx.add(i); reasons.append((i, f"50Hz-dominant(r={m['line_ratio']:.2f})")); continue
        if np.isfinite(m["hf_ratio"]) and m["hf_ratio"] > cfg["hf_ratio_max"]:
            bad_idx.add(i); reasons.append((i, f"HF-rauschig(r={m['hf_ratio']:.2f})")); continue
        if np.isfinite(m["jump_ratio"]) and m["jump_ratio"] > cfg["jump_ratio_max"]:
            bad_idx.add(i); reasons.append((i, f"zappelig(diff/std={m['jump_ratio']:.2f})")); continue
        bad_idx.add(i); reasons.append((i, "Pass1-ausgeschlossen")); continue
    if np.isfinite(m["corr"]) and m["corr"] < cfg["corr_min"]:
        bad_idx.add(i); reasons.append((i, f"schwache Mehrkanal-Korrelation(r={m['corr']:.2f})")); continue
...
_chan_filter_verbose = str(os.environ.get("CHAN_FILTER_VERBOSE", "0")).strip().lower() not in ("0", "false", "no", "off")
if _chan_filter_verbose:
    hdr = (f"[CHAN-FILTER][VERBOSE] thresholds: 50Hz<{cfg['line_ratio_max']:.2f} "
           f"HF<{cfg['hf_ratio_max']:.2f} corr>{cfg['corr_min']:.2f} "
           f"std_rel={cfg['std_rel_min']:.2f}-{cfg['std_rel_max']:.1f} "
           f"art<{cfg['artifact_frac']*100:.1f}%")
    print(hdr)
    for i in candidate_idx:
        m = _chan_metrics[i]
        status = "BAD " if i in bad_idx else "GOOD"
        print(
            f"[CHAN-FILTER][VERBOSE] pri_{i:02d} [{status}] "
            f"std_rel={m['rel_std']:.2f} "
            f"50Hz={m['line_ratio']:.3f} HF={m['hf_ratio']:.3f} "
            f"art={m['art_frac']*100:.1f}% "
            f"jmp={m['jump_ratio']:.3f} corr={m['corr']:.3f}"
        )
...
def _set_up_pair_preferring_raw(kU, kD, kU_raw, kD_raw):
    use_raw = str(os.environ.get("PREFER_RAW_UP_INDICES", "1")).strip().lower() not in ("0", "false", "no", "off")
    U_src = Up.get(kU_raw, []) if use_raw else Up.get(kU, [])
    D_src = Up.get(kD_raw, []) if use_raw else Up.get(kD, [])
    # Fallback auf nicht-raw, wenn raw nicht vorhanden/leer ist.
    if len(np.asarray(U_src).reshape(-1)) == 0 or len(np.asarray(D_src).reshape(-1)) == 0:
        U_src = Up.get(kU, [])
        D_src = Up.get(kD, [])
        used = "feature"
    else:
        used = "raw" if use_raw else "feature"
    Uc, Dc = _clip_pairs(U_src, D_src, nT)
    Up[kU], Up[kD] = Uc, Dc
    print(f"[UP-IDX] {kU}/{kD}: source={used}, n={len(Uc)}")

_set_up_pair_preferring_raw("Spontaneous_UP", "Spontaneous_DOWN", "Spontaneous_UP_raw", "Spontaneous_DOWN_raw")
_set_up_pair_preferring_raw("Pulse_triggered_UP", "Pulse_triggered_DOWN", "Pulse_triggered_UP_raw", "Pulse_triggered_DOWN_raw")
_set_up_pair_preferring_raw("Pulse_associated_UP", "Pulse_associated_DOWN", "Pulse_associated_UP_raw", "Pulse_associated_DOWN_raw")

# Konsistente Gesamtliste (raw-basiert nach obiger Auswahl), nach Onset sortiert.
_all_pairs = []
for kU, kD in [
    ...
    U = np.asarray(Up.get(kU, []), int)
    D = np.asarray(Up.get(kD, []), int)
    _all_pairs.extend([(int(u), int(d)) for u, d in zip(U, D) if d > u])
_all_pairs = sorted(set(_all_pairs), key=lambda p: p[0])
Up["UP_start_i"] = np.asarray([p[0] for p in _all_pairs], int) if _all_pairs else np.array([], int)
Up["DOWN_start_i"] = np.asarray([p[1] for p in _all_pairs], int) if _all_pairs else np.array([], int)
...
if np.isfinite(up_min_dur_post_s) and up_min_dur_post_s > 0:
    def _filter_short_up_pairs(U, D, min_dur_s, dt_s, label):
        ...
        m = min(U.size, D.size)
        if m == 0:
            print(f"[UP-FILTER-POST] {label}: kept=0 drop_short=0 (min_dur={min_dur_s:.3f}s)")
            return U[:0], D[:0]
        U, D = U[:m], D[:m]
        dur_s = (D - U).astype(float) * float(dt_s)
        keep = np.isfinite(dur_s) & (dur_s >= float(min_dur_s))
        dropped = int(np.sum(~keep))
        print(f"[UP-FILTER-POST] {label}: kept={int(np.sum(keep))} drop_short={dropped} (min_dur={min_dur_s:.3f}s)")
        return U[keep], D[keep]

    for kU, kD, lbl in [
        ("UP_start_i", "DOWN_start_i", "all"),
        ("Spontaneous_UP", "Spontaneous_DOWN", "spont"),
        ("Pulse_triggered_UP", "Pulse_triggered_DOWN", "trig"),
        ("Pulse_associated_UP", "Pulse_associated_DOWN", "assoc"),
    ]:
        Uf, Df = _filter_short_up_pairs(Up.get(kU, []), Up.get(kD, []), up_min_dur_post_s, dt, lbl)
        Up[kU], Up[kD] = Uf, Df
...
_amp_sig = main_channel_uV if main_channel_uV is not None else main_channel
spont_amp = _upstate_amplitudes(_amp_sig, Spon_UP_crop, Spon_DOWN_crop)
trig_amp  = _upstate_amplitudes(_amp_sig, Trig_UP_crop, Trig_DOWN_crop)
...
def detect_sharp_wave_ripple_intervals_in_upstates(
    signal_1d,
    time_s,
    dt,
    up_pairs,
...
try:
    ...
    scan_f_hi = float(os.environ.get("SWR_SCAN_F_HI_HZ", "300.0"))
    ...
    if n_scan > 0 and swr_enable:
        ...
        if pulse_times_1 is not None and len(pulse_times_1):
            pulse_on_all.append(np.asarray(pulse_times_1, float))
        ...
        for i_scan, ch_idx in enumerate(scan_channels):
            ...
            sig_plot = sig_raw
            if HTML_IN_uV:
                ch_name = chan_cols[ch_idx] if (0 <= ch_idx < len(chan_cols)) else None
                gain_ch = PER_CH_GAIN.get(ch_name, PREAMP_GAIN)
                if CALIB_MODE == "counts":
                    sig_plot = _counts_to_uV(sig_raw, ADC_BITS, ADC_VPP, gain_ch)
                elif CALIB_MODE == "volts":
                    sig_plot = _volts_to_uV(sig_raw)
                else:
            ...
            if swr_make_qa:
                for t0, t1 in swr_int:
                    if not np.isfinite(t0) or not np.isfinite(t1) or t1 <= t0:
                        continue
                    dur_ms = 1000.0 * (float(t1) - float(t0))
                    swr_durations_ms.append(float(dur_ms))
                    if pulse_on_all.size:
                        t_mid = 0.5 * (float(t0) + float(t1))
                        dt_near_ms = 1000.0 * float(np.min(np.abs(pulse_on_all - t_mid)))
                    else:
                        dt_near_ms = np.nan
                    swr_event_rows.append({
                        "channel_idx": int(ch_idx),
                        "t0_s": float(t0),
                        "t1_s": float(t1),
                        "duration_ms": float(dur_ms),
                        "mid_s": float(0.5 * (t0 + t1)),
            ...
            for i_scan, ch_idx in enumerate(scan_channels):
                ...
                for t0, t1 in swr_scan_intervals[i_scan]:
                    if np.isfinite(t0) and np.isfinite(t1) and t1 > t0:
                        ...
                        first = False
                if pulse_times_1 is not None and len(pulse_times_1):
                    ax.vlines(np.asarray(pulse_times_1, float), *ax.get_ylim(), lw=0.6, color="red", alpha=0.28, linestyles=":")
                if pulse_times_2 is not None and len(pulse_times_2):
    ...
    print(f"[WARN] SWR channel scan skipped: {e}")

spindle_min_dur_s = max(
    float(os.environ.get("SPINDLE_MIN_DUR_S", "0.45")),
    float(os.environ.get("SPINDLE_MIN_DUR_FLOOR_S", "0.35")),
)
spindle_min_cycles = max(
    int(os.environ.get("SPINDLE_MIN_CYCLES", "3")),
    int(os.environ.get("SPINDLE_MIN_CYCLES_FLOOR", "3")),
)
spindle_thr_on = max(
    float(os.environ.get("SPINDLE_THR_ON", "0.92")),
    float(os.environ.get("SPINDLE_THR_ON_FLOOR", "0.85")),
)
spindle_thr_off = max(
    float(os.environ.get("SPINDLE_THR_OFF", "0.30")),
    float(os.environ.get("SPINDLE_THR_OFF_FLOOR", "0.25")),
)
spindle_min_env_sigma = max(
    float(os.environ.get("SPINDLE_MIN_ENV_SIGMA", "0.72")),
    float(os.environ.get("SPINDLE_MIN_ENV_SIGMA_FLOOR", "0.65")),
)
spindle_use_psd_check = str(os.environ.get("SPINDLE_USE_PSD_CHECK", "0")).strip().lower() not in ("0", "false", "no", "off")
...
spindle_intervals_s = detect_spindle_intervals_in_upstates(
    ...
    min_env_peak_sigma=spindle_min_env_sigma,
...
print(
    "[SPINDLE-CONFIG] "
    f"thr_on={spindle_thr_on:.3f} "
    f"thr_off={spindle_thr_off:.3f} "
    f"min_dur={spindle_min_dur_s:.3f}s "
    f"max_gap={os.environ.get('SPINDLE_MAX_GAP_S', '0.12')}s "
    f"min_cycles={spindle_min_cycles} "
    f"min_env_sigma={spindle_min_env_sigma:.3f} "
    f"use_psd_check={int(spindle_use_psd_check)} "
...
if enable_swr:
    ...
    ripple_intervals_s = []
    print("[SWR] disabled for single-electrode mode (few channels).")
...
# Finale Spindle-Klassen strikt an UP-Klassen koppeln.
spindle_spont_s, spindle_trig_s, spindle_assoc_s, spindle_other_s = _classify_spindles_by_up_type(
    spindle_intervals_s,
    spont_up_s,
    trig_up_s,
    assoc_up_s,
...
# - triggered spindle nur in triggered UP
# - triggered UP nur mit triggered spindle
if str(os.environ.get("REQUIRE_TRIGGERED_SPINDLE_FOR_TRIGGERED_UP", "1")).strip().lower() not in ("0", "false", "no", "off"):
    trig_keep_u = []
    trig_keep_d = []
    trig_move_u = []
    trig_move_d = []
    m_trig = min(len(Pulse_triggered_UP), len(Pulse_triggered_DOWN))
    for u, d in zip(np.asarray(Pulse_triggered_UP[:m_trig], int), np.asarray(Pulse_triggered_DOWN[:m_trig], int)):
        if u < 0 or d <= u or d > len(time_s):
            continue
        t0 = float(time_s[u])
        t1 = float(time_s[d - 1]) if d - 1 < len(time_s) else float(time_s[-1])
        has_trig_sp = any(_intervals_overlap(t0, t1, s0, s1) for s0, s1 in spindle_trig_s)
        if has_trig_sp:
            trig_keep_u.append(int(u))
            trig_keep_d.append(int(d))
        else:
            trig_move_u.append(int(u))
            trig_move_d.append(int(d))

    if trig_move_u:
        assoc_u = np.asarray(Pulse_associated_UP, int)
        assoc_d = np.asarray(Pulse_associated_DOWN, int)
        assoc_pairs = {(int(u), int(d)) for u, d in zip(assoc_u, assoc_d)}
        for u, d in zip(trig_move_u, trig_move_d):
            assoc_pairs.add((int(u), int(d)))
        assoc_sorted = sorted(assoc_pairs, key=lambda p: p[0])
        Pulse_associated_UP = np.asarray([p[0] for p in assoc_sorted], int) if assoc_sorted else np.array([], int)
        Pulse_associated_DOWN = np.asarray([p[1] for p in assoc_sorted], int) if assoc_sorted else np.array([], int)

    Pulse_triggered_UP = np.asarray(trig_keep_u, int)
    Pulse_triggered_DOWN = np.asarray(trig_keep_d, int)
    Up["Pulse_triggered_UP"] = Pulse_triggered_UP
    Up["Pulse_triggered_DOWN"] = Pulse_triggered_DOWN
    Up["Pulse_associated_UP"] = Pulse_associated_UP
    Up["Pulse_associated_DOWN"] = Pulse_associated_DOWN

    # Nach möglichem Move UP-Intervalle neu bauen und Spindles final neu mappen.
    spont_up_pairs = _pair_up_down_indices(Spontaneous_UP, Spontaneous_DOWN, len(time_s))
    trig_up_pairs = _pair_up_down_indices(Pulse_triggered_UP, Pulse_triggered_DOWN, len(time_s))
    assoc_up_pairs = _pair_up_down_indices(Pulse_associated_UP, Pulse_associated_DOWN, len(time_s))
    spont_up_s = _pair_idx_to_time_intervals(spont_up_pairs, time_s)
    ...
    print(
        ...
        f"trig_up_kept={len(Pulse_triggered_UP)}",
        f"trig_up_moved_to_assoc={len(trig_move_u)}",
    )
...
ripple_assoc_s = []
ripple_spont_only_s = []
for t0, t1 in ripple_spont_s:
    if any(_intervals_overlap(t0, t1, a0, a1) for a0, a1 in assoc_up_s):
        ripple_assoc_s.append((float(t0), float(t1)))
    else:
        ripple_spont_only_s.append((float(t0), float(t1)))
ripple_spont_s = ripple_spont_only_s
...
Ripple_Spont_UP, Ripple_Spont_DOWN = _time_intervals_to_idx_pairs(ripple_spont_s, time_s)
Ripple_Trig_UP, Ripple_Trig_DOWN = _time_intervals_to_idx_pairs(ripple_trig_s, time_s)
Ripple_Assoc_UP, Ripple_Assoc_DOWN = _time_intervals_to_idx_pairs(ripple_assoc_s, time_s)
...
print(
    ...
    f"swr_spont={len(Ripple_Spont_UP)}",
...
try:
    ...
    n_ch_total = int(LFP_array.shape[0]) if (hasattr(LFP_array, "shape") and len(LFP_array.shape) >= 2) else 0
    if n_ch_total <= 0:
        raise ValueError("LFP_array has no channel dimension.")

    swr_ch_idx = int(np.clip(req_swr_ch, 0, n_ch_total - 1))
    up_ch_idx = int(np.clip(req_up_ch, 0, n_ch_total - 1))
    if swr_ch_idx != req_swr_ch:
        print(f"[WARN] requested SWR channel pri_{req_swr_ch} not available -> using pri_{swr_ch_idx}")
    if up_ch_idx != req_up_ch:
        print(f"[WARN] requested UP/SP channel pri_{req_up_ch} not available -> using pri_{up_ch_idx}")

    def _channel_signal_for_html(ch_idx):
        sig = np.asarray(LFP_array[int(ch_idx)], dtype=float)
        if not HTML_IN_uV:
            return sig
        ch_name = chan_cols[int(ch_idx)] if (0 <= int(ch_idx) < len(chan_cols)) else None
        gain_ch = PER_CH_GAIN.get(ch_name, PREAMP_GAIN)
        if CALIB_MODE == "counts":
            return _counts_to_uV(sig, ADC_BITS, ADC_VPP, gain_ch)
        if CALIB_MODE == "volts":
...
try:
    ...
    if preferred_second_ch in set(map(int, good_idx)):
        ...
        deeper_candidates = [int(g) for g in good_idx if int(g) > int(ch_idx_used)]
        if deeper_candidates:
            second_ch_idx = int(deeper_candidates[-1])  # möglichst weit unten
        else:
            fallback_candidates = [int(g) for g in good_idx if int(g) != int(ch_idx_used)]
            second_ch_idx = int(fallback_candidates[-1]) if fallback_candidates else int(ch_idx_used)
        print(f"[WARN] preferred channel pri_{preferred_second_ch} not in good_idx -> fallback to pri_{second_ch_idx}")

    second_channel = np.asarray(LFP_array[second_ch_idx], dtype=float)
    second_channel_uV = None
    if HTML_IN_uV:
        second_name = chan_cols[second_ch_idx] if (0 <= second_ch_idx < len(chan_cols)) else None
        gain_second = PER_CH_GAIN.get(second_name, PREAMP_GAIN)
        if CALIB_MODE == "counts":
            second_channel_uV = _counts_to_uV(second_channel, ADC_BITS, ADC_VPP, gain_second)
        elif CALIB_MODE == "volts":
            second_channel_uV = _volts_to_uV(second_channel)
        elif CALIB_MODE == "uV":
            second_channel_uV = second_channel.copy()
        else:
            second_channel_uV = second_channel.copy()
