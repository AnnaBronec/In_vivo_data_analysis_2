...
def plot_all_channels(
    ...
    pulse_times_1: np.ndarray | None = None,
    pulse_times_2: np.ndarray | None = None,
    label_1: str = "Light pulses 1",
    label_2: str = "Light pulses 2",
    figsize=(12, 1.6),
):
    """
    Plottet alle Kanäle untereinander und overlayed optional Lichtpulse als vertikale Linien.
    - time_s: Zeitvektor (Sekunden), shape (T,)
    - LFP_array: shape (C, T) (Kanäle x Zeit)
    - pulse_times_{1,2}: Sekunden (gleiche Zeitbasis wie time_s!)
    """
    C, T = LFP_array.shape
    ...
    tmin, tmax = float(np.nanmin(time_s)), float(np.nanmax(time_s))

    def _mask_in_range(pulses):
        if pulses is None or len(pulses) == 0:
            return None
        p = np.asarray(pulses, dtype=float)
        return p[(p >= tmin) & (p <= tmax)]

    p1 = _mask_in_range(pulse_times_1)
    p2 = _mask_in_range(pulse_times_2)

    for i in range(n):
        ax = axes[i]
        y = LFP_array[i, :]
        ax.plot(time_s, y, lw=0.6)
        ax.set_ylabel(f"pri_{i}", rotation=0, ha="right", va="center")
        ax.grid(True, alpha=0.3)

        ymin, ymax = ax.get_ylim()
        if p1 is not None and p1.size:
            ax.vlines(p1, ymin=ymin, ymax=ymax, linewidth=0.8, alpha=0.6, label=label_1)
        if p2 is not None and p2.size:
            ax.vlines(p2, ymin=ymin, ymax=ymax, linewidth=0.8, alpha=0.6, linestyles="dashed", label=label_2)

        if i == 0 and ((p1 is not None and p1.size) or (p2 is not None and p2.size)):
            ax.legend(loc="upper right", frameon=False, fontsize=8)

    axes[-1].set_xlabel("Time (s)")
    plt.suptitle("Plot 1 – LFP-Kanäle über Zeit (mit Lichtpulsen)", y=0.995)
    plt.show()
...
def plot_spont_up_mean(main_channel, time_s, dt, Spon_Peaks, up_state_binary,
    ...
    plt.title("UP-States: spontaneous (blau) vs. triggered (red)")
    plt.tight_layout()
    plt.show()
